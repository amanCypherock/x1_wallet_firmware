...
extern void initiator_listener();
/**
 ...
 * device initiates pairing by sending pairing data including signature and card returns its pairing data,
 * nfc_pair is used to exchange pairing data, data_inOut and length_inOut should contain pairing data and its size,
 * pairing data returned by card is copied to the buffer if pairing was successful on card
 *
 * @param [in/out] data_inOut pointer to buffer with input and output data
 * @param [in,out] length_inOut pointer to variable with length of send and receive data
 *
 ...
 * 
 * @param[in,out]   expected_family_id  Any card with family ID other than the provided ID will not be accepted
 ...
 * 
 * @param[in,out]   data_inOut  Data to be signed as input and Signed data as output
 *
 * @return
 * @retval
 *
 ...
 * @param[in,out] recv_len Expected length of received data and Length of received data
 * 
 * @returns ret_code_t STM_ERROR_CODE
 * @retval
 *
