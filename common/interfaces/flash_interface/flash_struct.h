 ...
 * @details The data needed to prove the device has done the work and unlock the card is stored here.
 *
 * @see
 * @since v1.0.0
 *
...
typedef struct Flash_Wallet {
    ...
    uint8_t wallet_info;
    uint8_t wallet_share_with_mac_and_nonce[BLOCK_SIZE + NONCE_SIZE + WALLET_MAC_SIZE];	//does not include MAC and nonce

    uint8_t state;		 // if DEFAULT_VALUE_IN_FLASH then not valid. If equal to VALID_WALLET then valid
    uint8_t cards_states; 	 // ith from right ((cards_states>>i)&1) bit tells whether card (i+1) has the share or not.
